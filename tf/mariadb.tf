...
# resource "azurerm_mariadb_virtual_network_rule" "mariadb_admin" {
#   count               = local.slurm_accounting ? 1 : 0
#   name                = "AllowAccessAdmin"
#   resource_group_name = azurerm_mariadb_server.mariadb[0].resource_group_name
#   server_name         = azurerm_mariadb_server.mariadb[0].name
#   subnet_id           = local.create_admin_subnet ? azurerm_subnet.admin[0].id : data.azurerm_subnet.admin[0].id
# }

# resource "azurerm_mariadb_virtual_network_rule" "mariadb_frontend" {
#   count               = local.slurm_accounting ? 1 : 0
