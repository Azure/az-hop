...
resource "azurerm_virtual_network_peering" "azhop-to-peer" {
  count                         = length(local.vnet_peering)
  name                          = local.vnet_peering[count.index].vnet_resource_group #"azhop-to-${local.vnet_peering[count.index].vnet_name}"
...
resource "azurerm_virtual_network_peering" "peer-to-azhop" {
  count                         = length(local.vnet_peering)
  name                          = local.create_rg ? azurerm_resource_group.rg[0].name : data.azurerm_resource_group.rg[0].name #"${local.vnet_peering[count.index].vnet_name}-to-azhop"
  resource_group_name           = local.vnet_peering[count.index].vnet_resource_group
