  ...
  content = templatefile("${local.playbooks_template_dir}/global_variables.tmpl",
    {
      ...
      database-fqdn       = local.slurm_accounting ? azurerm_mariadb_server.mysql[0].fqdn : ""
      database-ip         = local.slurm_accounting ? azurerm_private_endpoint.mariadb[0].private_service_connection[0].private_ip_address : ""
      database-user       = local.slurm_accounting_admin_user
      jumpbox-ssh-port    = local.jumpbox_ssh_port
    }
  )
