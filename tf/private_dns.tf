...
resource "azurerm_private_dns_a_record" "ad" {
  ...
  ttl                 = 3600
  records             = local.create_ad || local.use_existing_ad? [local.domain_controller_ips[0]] : []
}
...
resource "azurerm_private_dns_a_record" "ad2" {
  ...
  zone_name           = azurerm_private_dns_zone.azhop_private_dns.name
  ttl                 = 3600
  records             = local.create_ad || local.use_existing_ad? [try(local.domain_controller_ips[1], local.domain_controller_ips[0])] : []
}
