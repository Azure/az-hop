...
jq '(.definition.plans[] | select(.planId==$plan)."microsoft-azure-virtualmachines.vmImages") += $version ' --arg plan ${os} --argjson version "$(jq '.' temp.json)" ./offers/azhop-${offer}-${os}.json > ./offers/azhop-${offer}-${os}-final.json
